...
ts = (pd.to_datetime(df_price["timestamp"], utc=True, errors="coerce").dt.tz_convert(None))
latest_ts = ts.max()
since_time = latest_ts - timedelta(hours=hours)
...
df_mentions = df.groupby(pd.Grouper(key="minute", freq="10min")).size().reset_index(name="mentions").rename(
    columns={"minute": "time"}).set_index("time")
...
with col3:
    styled_df = (df_latest.style.format({"price": "{:.2f}"}).set_table_styles([
        {'selector': 'thead',
         'props': [('color', '#48cae4'), ('background-color', '#111936'), ("border", "2px solid #2a2e45"),
                   ('font-weight', 'bold')]},
        {'selector': 'tbody td',
         'props': [('color', '#FFFFFF'), ('background-color', '#111936'), ("border", "2px solid #2a2e45"),
...
df_mentions_total = df_mentions_total.groupby("ticker").size().reset_index(name="mentions").sort_values("mentions",
                                                                                                        ascending=False)
