...
latest_ts = ts.max()
since_time = latest_ts - timedelta(hours=1)

SEQ_LEN = 10
MODEL_DIR = Path(__file__).resolve().parents[2] / "demo_models"
...
fig.add_trace(
    go.Scatter(x=val_df["time"], y=val_df["pred"], name="Predicted", line=dict(color="#E040FB", width=3, dash="dash")))
fig.update_layout(
    height=300,
...
fig = go.Figure()
fig.add_trace(
    go.Scatter(x=df_price.index, y=df_price['price'], name="Actual Price", line=dict(color="#48cae4", width=3)))
fig.add_trace(
    go.Scatter(x=actual_px.index, y=pred_px, name="Predicted Price", line=dict(color="#E040FB", width=3, dash="dash")))
